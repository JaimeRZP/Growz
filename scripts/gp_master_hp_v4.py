...
with pm.Model() as model:
    ...
    H0 = data_class.H0
    Wm0 = data_class.Wm0 #pm.Uniform("Wm0", 0., 1.) 
    ...
    if get_fs8:
        #s80 = data_class.s80
        s80 = data_class.cosmo.sigma8() #pm.Normal("s80", 0.8, 0.5)
        E = H_gp/H_gp[0]
        xx = x_arr[::-1]
        ee = E[::-1]
        aa = np.exp(-xx)
...
#print r-stat
print(pm.summary(trace)['r_hat'][["η"]])
print(pm.summary(trace)['mean'][["η"]])
...
filename += '_noWmnos8_l_{}_{}_{}'.format(ℓ, n_samples, n_tune)
print(filename)
