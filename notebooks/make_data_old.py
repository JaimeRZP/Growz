...
class make_data():
    def __init__(self, z_max, dz, path):
        self.path = path
        self.tools = utils_old.utils()
        self.dz = dz
        self.z_max = z_max
    ...
    def get_DESI(self, z_arr, new='False'):
        ...
        if (os.path.exists(filepath)) and (new is False):
            ...
            z_DESI = np.arange(0.15, 1.85+0.1, 0.1)+(0.05)*np.random.randn(18)
            DESI_idx =  self.make_idx(z_DESI, z_arr)
            DESI_U = self.make_U(z_DESI, z_arr, DESI_idx)
            
            signal = self.gaussian(z_arr, 0.8, 0.2) 
            H_arr_f = self.Planck['Hkms_arr']*(1+0.2*signal)
            dA_arr_f = self.tools.make_dM((1000/self.tools.c)*H_arr_f, z_arr)
            dA_arr_f[1:] /= (1+self.z_arr)[1:]
            f_arr_f = self.tools.make_f(H_arr_f, z_arr, 0.31, 0.44)
            s8_arr_f = self.tools.make_sigma8(f_arr_f, z_arr, 0.805)
    ...
    def get_CC(self, z_arr, new='False'):
        ...
        if (os.path.exists(filepath)) and (new is False):
            ...
            signal = self.gaussian(z_arr, 0.8, 0.2)
            signal_i = interpolate.interp1d(z_arr, signal)
            CC_data = CC_data*(1+0.2*signal_i(z_CC))
